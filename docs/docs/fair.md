...
- A **DOI** assigned through Zenodo, providing a globally unique, persistent identifier.
...
- Stored in open repositories (Zenodo) that use standard HTTPS access.
...
- Maps to Zenodo's native format without loss, so metadata transfers cleanly between systems.
